...
class Loader:
    ...
    def _load(data: dict) -> LootDrop|Item:
        if "entries" in data.keys():
            ...
            roll_args: List[int] = [rolls[f"arg{i}"] for i in range(arg_count)]
            roll_func: Callable = lambda: RollType[rolls["type"]](*roll_args)
            ...
            chance_args: List[int|List[int]] = [chance[f"arg{i}"] for i in range(arg_count-1)]
            chance_func: Callable = lambda: ChanceType[chance["type"]](size, *chance_args)
